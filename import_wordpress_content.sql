...
LEFT JOIN wp_postmeta AS yoast_title
    ON yoast_title.post_id = p.ID
    AND yoast_title.meta_key = '_yoast_wpseo_title'

LEFT JOIN wp_postmeta AS yoast_description
...
INNER JOIN content_categories AS cc
...
INNER JOIN wp_postmeta AS thumbnail
    ON thumbnail.post_id = p.ID
    AND thumbnail.meta_key = '_thumbnail_id'

INNER JOIN wp_postmeta AS attached_file
